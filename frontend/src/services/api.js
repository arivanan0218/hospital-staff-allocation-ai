...
export const staffAvailabilityAPI = {
  ...
  getAvailability: (staffId) => api.get(`/staff/availability/${staffId}`),
  
  // Get all available staff
  getAvailableStaff: () => api.get('/staff/available/current'),
  
  // Update staff availability
  updateAvailability: (staffId, availabilityData) => 
    api.put(`/staff/availability/${staffId}`, availabilityData),
  
  // Get working staff
  getWorkingStaff: () => api.get('/staff/working'),
  
  // Get availability timeline
  getTimeline: (staffId) => api.get(`/staff/availability/${staffId}/timeline`)
};

// Shift status API endpoints
export const shiftStatusAPI = {
  // Update shift status
  updateStatus: (shiftId, status, data = {}) => 
    api.put(`/shifts/${shiftId}/status`, { status, ...data }),
  
  // Get active shifts
  getActiveShifts: () => api.get('/shifts/active'),
  
  // Get ending soon shifts
  getEndingSoon: (hoursAhead = 2) => 
    api.get(`/shifts/ending-soon?hours=${hoursAhead}`),
  
  // Start shift
  startShift: (shiftId) => 
    api.post(`/shifts/${shiftId}/start`),
  
  // Complete shift
  completeShift: (shiftId, notes = '') => 
    api.post(`/shifts/${shiftId}/complete`, { completion_notes: notes }),
  
  // Extend shift
  extendShift: (shiftId, newEndTime, reason = '') => 
    api.post(`/shifts/${shiftId}/extend`, { new_end_time: newEndTime, reason })
};

// Attendance API endpoints
export const attendanceAPI = {
  // Check in staff
  checkIn: (staffId, shiftId, location = null) => 
    api.post(`/staff/${staffId}/checkin`, { shift_id: shiftId, location }),
  
  // Check out staff
  checkOut: (staffId, shiftId, notes = '') => 
    api.post(`/staff/${staffId}/checkout`, { shift_id: shiftId, notes }),
  
  // Get attendance records
  getAttendance: (staffId, dateRange = null) => {
    const params = dateRange ? `?date_range=${dateRange}` : '';
    return api.get(`/staff/${staffId}/attendance${params}`);
  },
  
  // Get current status
  getCurrentStatus: (staffId) => api.get(`/staff/${staffId}/current-status`)
};